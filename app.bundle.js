...
$(function () {
  ...
  let circleBars = [];
  $('.progressbar-wrapper .circle-container').each(function (index, value) {
  ...
  $(window).scroll(function () {
    let scroll = $(window).scrollTop() + $(window).height(); //Если скролл до конца елемента
    //var offset = $element.offset().top + $element.height();
    //Если скролл до начала елемента

    let offset = $element.offset().top;
    let elheigtn = $element.height() / 2; // console.log(offset,scroll);

    if (!animated && animatedFinished && scroll > offset + elheigtn) {
      animated = true;
      animatedFinished = false;
      let m = $(".procent").each(function (index, val) {
        animationPercentage = $(val).attr('data-procent') / 100 || animationPercentage; // console.log('anim', animationPercentage);
  ...
  $('.arrow-map').on('click', function () {
    $('.map-text').css('display', 'none');
    $('.map-after').css('display', 'none');
  });
  $('header .arrow-box').on('click', function (e) {
    e.preventDefault();
    $('body,html').animate({
      scrollTop: 800
    }, 400);
  ...
  var tab = function (selector_link, selector_content) {
